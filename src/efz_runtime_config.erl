...
defaults() ->
    #{enabled=>false,
      ...
      resources=>#{enabled=>true,sample_interval_ms=>20,max_samples=>64,
          ...
          residual_memory_bytes=>67108864,quiescence_ms=>1,memory_observation_timeout_ms=>50},
...
prepare(C) when is_map(C) ->
    try
        ...
        [range(P,K,1,1099511627776)||K<-[memory_bytes,mailbox_messages,ets_memory_bytes,residual_memory_bytes]],
        range(P,quiescence_ms,0,100),range(P,memory_observation_timeout_ms,1,1000),
        true=maps:get(memory_observation_timeout_ms,P)>maps:get(quiescence_ms,P),
